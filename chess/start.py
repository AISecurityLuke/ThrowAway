...
class Game:
    ...
    def launch_gui(self):
        ...
        # Create an 8x8 grid of buttons. In the GUI we want white pieces to appear at the bottom,
        # so we map the GUI grid row to board row with: board_row = 7 - gui_row.
    ...
    def is_game_over(self):
        ...
        """
        # Placeholder: game never ends in this basic version
        return False

    def process_move(self, move, ai_move=False):
    ...
    def on_square_click(self, gui_r, gui_c):
        ...
        # Map GUI grid coordinates to board coordinates (remember: board_row = 7 - gui_r)
        board_r = 7 - gui_r
        board_c = gui_c
        ...
        if self.selected_square is None:
            ...
            self.update_gui()
            # After a human move, if playing versus AI and it is now the AI's turn, schedule the AI move.
            if self.vs_ai and self.turn == self.ai_color:
    ...
    def try_move(self, move_str, ai_move=False):
        ...
        result = self.process_move(move_str, ai_move=ai_move)
...
if __name__ == "__main__":
    ...
    if ai_choice == "Y":
        ...
        # Ensure that the human player starts if they choose white; otherwise, let AI start.
        game.turn = "white" if player_color == "white" else "black"
    if mode == "2":
        game.launch_gui()
    else:
        game.play()
